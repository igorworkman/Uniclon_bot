...
- `modules/generate_copy.sh` — модуль генерации копий видео. Реализует основную логику: FFmpeg фильтры, crop, аудио обработку, EXIF, превью, метрики и fallback.
...
- `timeout` — Используется для защиты FFmpeg от зависаний (max 300 с).
### 🧪 tests/
<!-- REGION AI: tests scripts update -->
- `tests/selfcheck.sh` — автоматическая проверка стабильности пайплайна Uniclon.
- `tests/check_all.sh` — автоматическая комплексная проверка синтаксиса, импортов, зависимостей и тестов.
<!-- END REGION AI -->
### 🧰 utils/
