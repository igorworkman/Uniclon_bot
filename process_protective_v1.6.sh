...
# END REGION AI
  ...
  if ! _ffmpeg_retry "${FFMPEG_RETRY_COUNT:-3}" "${FFMPEG_RETRY_DELAY:-1}" bash -c "$FFMPEG_CMD"; then
...
report_builder_reset "$OUTPUT_DIR"

for idx in "${!RUN_FILES[@]}"; do
  manifest_write_entry "$MANIFEST_PATH" "$idx" "$validated_flag" "$regen_flag"
  quality_flag="${RUN_QPASS[$idx]:-false}"
