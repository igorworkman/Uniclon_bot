...
clip_start() {
  ...
  local copy_id="${4:-}"
  local seconds="" fallback_seconds="" fallback_display=""
  ...
  if ! fallback_display=$(printf "%.3f" "$fallback_seconds" 2>/dev/null); then
    fallback_display="0.000"
  fi

  echo "⚠️ [$label] Некорректное значение '$value' — используется fallback=${fallback_display} (${copy_id})"
    ...
    if variant_payload=$(python3 "$BASE_DIR/modules/utils/video_tools.py" generate \
      ...
      while IFS= read -r variant_line; do
        ...
        if [[ "$variant_line_trimmed" == Adjusted:* ]]; then
          echo "Adjusted: ${variant_line_trimmed#Adjusted:}"
          continue
        elif [[ "$variant_line_trimmed" == Adjusted* ]]; then
          echo "Adjusted: ${variant_line_trimmed#Adjusted}"
          continue
        fi
